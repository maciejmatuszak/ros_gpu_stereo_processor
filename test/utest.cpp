...
class CudaStereoBMTf : public testing::Test
{
  protected:
    ...
    void initStereoModelLab()
    {
        auto left_file = (test_data_path / "stereobm/left_info.yaml").string();
        auto right_file = (test_data_path / "stereobm/right_info.yaml").string();
...
TEST_F(CudaStereoBMTf, RectifyMonoGpu)
{
    ...
    cv::Mat l_rect_gpu, r_rect_gpu, disparity;
    stereo_processor_.computeDisparity(GPU_MAT_SRC_L_RECT_MONO, GPU_MAT_SRC_R_RECT_MONO, GPU_MAT_SRC_L_DISPARITY);
    ...
    createStereoWithEpipolar(l_rect_gpu, disparity, (test_data_path / "stereobm/RectifyMonoGpu_rect_disp.png").string());
    ...
    cv::imwrite((test_data_path / "stereobm/RectifyMonoGpu_l_disp.png").string(), disparity);
