...
GPUSender::GPUSender(GpuStereoProcessor *gpuProcessor, const std_msgs::Header &header, ros::Publisher *pub)
    : publisher_(pub)
    , data_sent(false)
    , header_(header)
...
GPUSender::GPUSender(GpuStereoProcessor *gpuProcessor, sensor_msgs::ImageConstPtr example, std::string encoding, ros::Publisher *pub)
    : publisher_(pub)
    , data_sent(false)
    , gpuProcessor_(gpuProcessor)
{
...
GPUSender::GPUSender(GpuStereoProcessor *gpuProcessor, sensor_msgs::ImageConstPtr example, int blockSize, int numDisparities, int minDisparity, double fx, double baseline,
                     ros::Publisher *pub)
    : publisher_(pub)
    , data_sent(false)
    , gpuProcessor_(gpuProcessor)
...
inline bool GPUSender::isValidPoint(const cv::Vec3f &pt)
{
...
void GPUSender::fillInPointMessage()
{
    // fill in points
...
void GPUSender::send(void)
{
    ...
    {
        gpuProcessor_->filterSpeckles(cpu_data_);
...
void GPUSender::enqueueSend(cv::cuda::GpuMat &m, cv::cuda::GpuMat &col, cv::cuda::Stream &strm)
{
...
void GPUSender::enqueueSend(cv::cuda::GpuMat &m, cv::cuda::Stream &strm)
{
...
cv::Mat &GPUSender::getCpuData() { return cpu_data_; }

sensor_msgs::PointCloud2Ptr &GPUSender::getPointsMessage() { return points2_msg_; }

bool GPUSender::wasDataSent() { return data_sent; }
}
