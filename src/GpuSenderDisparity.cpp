...
GPUSenderDisparity::GPUSenderDisparity(const std_msgs::Header *header, const ros::Publisher *pub, boost::shared_ptr<cv::cuda::HostMem> imageHMem, int blockSize, int numDisparities,
...
void GPUSenderDisparity::publish()
{
    if (publisher_)
    {
        publisher_->publish(disparity_msg_);
    }
}

boost::shared_ptr<cv::cuda::HostMem> GPUSenderDisparity::getDisparityHostMem() { return disparityHMem_; }
